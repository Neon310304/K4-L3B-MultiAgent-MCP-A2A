...
async def _show_tools(root: Path, *, details: bool = False) -> None:
    ...
    async with connect_gateway(settings.mcp_endpoint, settings.team_api_key, contracts) as gateway:
        ...
        for name, spec in sorted(catalog.items()):
            if details:
                print(json.dumps({"name": name, "input_schema": spec.input_schema}, indent=2))
            else:
                print(name)
    ...
    async with connect_gateway(settings.mcp_endpoint, settings.team_api_key, contracts) as gateway:
        ...
        with TemporaryDirectory(prefix=".day09-run-", dir=root) as temporary_root:
            ...
            for index, case_id in enumerate(case_set.case_ids, 1):
                print(f"RUN: {index:03d}/{total:03d} {case_id}", flush=True)
            ...
            staged = list(staging_outputs.glob("*.json"))
            if len(staged) != total:
                raise RuntimeError(f"batch produced {len(staged)} of {total} outputs")
            for stale in output_root.glob("*.json"):
                stale.unlink()
            for source in staged:
                source.replace(output_root / source.name)
            staging_trace.replace(trace_path)
            print(f"OK: generated {total} outputs and {trace_path}")
...
def parser() -> argparse.ArgumentParser:
    ...
    tools.add_argument("--details", action="store_true", help="include discovered input schemas")
